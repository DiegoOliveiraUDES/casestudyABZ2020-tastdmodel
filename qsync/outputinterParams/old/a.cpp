...
{
	int  exec = 0;
	for(auto const& _z_ : ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].f)
	{
		z = _z_.first;
		if(0)
		{
...
{
	int  exec = 0;
	for(auto const& _y_ : ts_A.f[ts_A.x].f)
	{
		y = _y_.first;
...
int  exists_Ce_z(int&  z, int  _z)
{
	if(std::find(T_C.begin(), T_C.end(), _z) == T_C.end())
	{
		return 0;
	...
	{
		if((std::find(T_C.begin(), T_C.end(), z) !=T_C.end() && ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].f[ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].z].autState == S0))
		{
			return 1;
...
int  exists_Be_y(int&  y, int&  z, int  _y, int  _z)
{
	if(std::find(T_B.begin(), T_B.end(), _y) == T_B.end())
	{
		return 0;
	...
	{
		if((std::find(T_B.begin(), T_B.end(), y) !=T_B.end() && exists_Ce_z(ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].z, _z)))
		{
			return 1;
	...
	}else 
	{
		...
		if(exists_Ce_z(ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].z, _z))
		{
			return 1;
...
int  exists_Ae_x(int&  x, int&  y, int&  z, int  _x, int  _y, int  _z)
{
	if(std::find(T_A.begin(), T_A.end(), _x) == T_A.end())
	{
		return 0;
	...
	{
		if((std::find(T_A.begin(), T_A.end(), x) !=T_A.end() && exists_Be_y(ts_A.f[ts_A.x].y, ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].z, _y, _z)))
		{
			return 1;
	...
	}else 
	{
		...
		if(exists_Be_y(ts_A.f[ts_A.x].y, ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].z, _y, _z))
		{
			return 1;
...
int    e(int  _x, int  _y, int  _z)
{
	int  exec = 0;
	if(exists_Ae_x(ts_A.x, ts_A.f[ts_A.x].y, ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].z, _x, _y, _z))
	{
		if(exists_Be_y(ts_A.f[ts_A.x].y, ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].z, _y, _z))
		{
			if(exists_Ce_z(ts_A.f[ts_A.x].f[ts_A.f[ts_A.x].y].z, _z))
...
int main(int argc, char** argv)
{
	...
	while (1)
	{
		...
		{
			if(e(Types::get_int(_evt.params[0]), Types::get_int(_evt.params[1]), Types::get_int(_evt.params[2])))
			{
